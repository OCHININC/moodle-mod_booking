...
if (!$tableallbookings->is_downloading()) {
    ...
    // Conditionally add enrollment fields and JOIN if courseenrolmethod is configured to show.
    $enrolmentfields = '';
    $enrolmentjoin = '';
    if (in_array('courseenrolmethod', $responsesfields)) {
        $enrolmentfields = ',
        ...
        $enrolmentjoin = '
            LEFT JOIN (
                SELECT
                    ue1.userid,
                    e1.courseid,
                    e1.enrol AS enrolmethod,
                    e1.customint1 AS cohortid,
                    ROW_NUMBER() OVER (
                        PARTITION BY ue1.userid, e1.courseid
                        ORDER BY CASE e1.enrol
                            WHEN \'cohort\' THEN 1
                            WHEN \'manual\' THEN 2
                            WHEN \'self\' THEN 3
                            ELSE 4
    ...
    // Conditionally add booking history fields and JOIN if bookingsource is configured to show.
    $bookinghistoryfields = '';
    $bookinghistoryjoin = '';
    if (in_array('bookingsource', $responsesfields)) {
        $bookinghistoryfields = ',
            CASE
    ...
    $fields = 'ba.id, ' . $mainuserfields . ',
            ...
            . $enrolmentfields
            . $bookinghistoryfields
            . $customfields . $shoppingcartfields . $certificatefields;
